"""Render scripted-controller episodes for each task to GIFs for the README.
...
def render_task(name: str, env_id: str, seed: int = 0, max_steps: int = 200) -> list[Image.Image]:
    env = gym.make(
        env_id,
        obs_mode="detection",
...
if __name__ == "__main__":
    for name, env_id in TASKS.items():
        frames = render_task(name, env_id)
